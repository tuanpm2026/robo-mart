...
CI fixed: the old `Integration Tests` step (which skipped failsafe via `-DskipTests` and disabled JaCoCo) was replaced. Backend CI now runs a fast `build` job (compile + Checkstyle + unit-only coverage gate) that gates a **parallel per-service `integration-tests` matrix** — each IT-bearing service (product / cart / order / inventory / payment / notification / events) runs on its OWN isolated runner so no service's Testcontainers contend with another's. This per-service split removed the cross-service resource contention that was starving Elasticsearch and making product-service search ITs flaky in CI. The JaCoCo threshold stays at **0.80** and is now enforced for every module; all clear it (api-gateway raised 0.60 → 1.00 via `UserIdRelayFilterUnitTest`).

Two within-service ES IT isolation bugs were also fixed (the matrix split alone could not — both are inside product-service): a poisoned shared-Redis `@Cacheable` search cache (an empty result cached for 60s during an ES near-real-time gap caused later identical queries to hit the stale empty cache and time out) and cross-class index contamination from the live `ProductIndexConsumer` indexing sibling-IT products into the shared `products` index. Fixed by clearing the caches per class, pausing the consumer for the class lifetime, seeding once with `RefreshPolicy.IMMEDIATE`. Backend CI is green end-to-end.
...
| SCHEMA-CI | 2026-06-20 CI | `schema-compatibility.yml` (buf lint + Avro backward-compat vs Schema Registry) has never passed — it also fails on `main` (last run 2026-04-20), independent of Backend CI. | Pre-existing, unrelated to the integration-test work; NOT a regression from this PR. Investigate the buf lint config and the Avro compatibility baseline separately. |
