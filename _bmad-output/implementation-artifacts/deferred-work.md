...
- **ReleaseInventoryStep iterates order.getItems() without lazy-load guard in stale saga recovery**: `recoverStaleSagas()` passes a JPA entity to `releaseInventoryStep.compensate()` without guaranteeing `items` are eagerly loaded. If the items collection is lazy and the session is closed, an empty (or NPE-triggering) items list is sent to the Inventory Service, leaving inventory permanently reserved. Add `@Transactional` or explicit item loading in `recoverStaleSagas()`.
- **DEADLINE_EXCEEDED on payment charges customer with no refund**: If `processPayment()` succeeds at the Payment Service but the gRPC response times out (DEADLINE_EXCEEDED), `ProcessPaymentStep` treats it as a failure with `shouldCompensate=true`. `OrderSagaOrchestrator.runCompensation()` only calls `releaseInventoryStep.compensate()` — `refundPaymentStep` is never invoked. The customer is charged but no refund is issued. Requires saga phase B work (Story 8.4) to track confirmed payment IDs before triggering compensation.
- **Refund reason is generic/null during stale saga recovery**: For stale orders in `PAYMENT_REFUNDING` state recovered by `recoverStaleSagas()`, `order.getCancellationReason()` may be null. `RefundPaymentStep` falls back to the generic `"Order cancelled"` reason when sending the refund to Payment Service. Store the actual cancellation reason before transitioning to PAYMENT_REFUNDING.
