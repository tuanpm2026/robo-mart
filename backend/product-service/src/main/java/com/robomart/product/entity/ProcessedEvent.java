 ...
 * group, so at-least-once redelivery does not reprocess it (redundant Elasticsearch re-index / cache
 * eviction). The unique {@code (consumer_group, event_id)} constraint is the idempotency key — keyed
 * per group so the same event consumed by independent consumer groups is processed once each.
 */
@Entity
