 ...
 * boundary: {@code /api/v1/admin/**} requires the ADMIN role and all other order endpoints require
 * authentication, so a direct in-cluster call cannot bypass the gateway's RBAC. HTTP only — the
 * gRPC server is protected by the NetworkPolicy (ingress restricted to callers, not exposed).
 */
@Configuration
@EnableWebSecurity
